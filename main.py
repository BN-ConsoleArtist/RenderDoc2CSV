        ...
        with open(file_path, newline='') as file:
            ...
            try:
                with open(export_path, mode='w', newline='') as f:
                    ...
                    log_callback("Export completed (" + str(f_export) + ")\n")
                    log_callback("Summary:")
                    log_callback(f'Opaque Drawcalls = {drawcall_count_opaque}')
                    log_callback(f'Translucency Drawcalls = {drawcall_count_translucency}\n')
                    log_callback(f'Total Drawcalls = {drawcall_count_opaque + drawcall_count_translucency}')
...
class Window(QMainWindow):
    def __init__(self):
       ...
       # Set the window title and initial size
        ...
        self.button3.clicked.connect(self.run_export)
    ...
    def get_file(self):
        ...
        file_path = QFileDialog.getOpenFileName(filter="*.txt")[0] #string type
        if not file_path:
            self.terminal.appendPlainText("File selection cancelled...")
            return

        self.path_import = pathlib.Path(file_path)
        self.display_path_import.setText(str(self.path_import))
        self.path_export = self.path_import.with_suffix('.csv')
        self.display_path_export.setText(str(self.path_export))
        self.terminal.appendPlainText(str(self.path_import) + " selected...")
        if self.path_export.is_file():
            self.terminal.appendPlainText("Warning, CSV file already exists and will be overwritten...")
